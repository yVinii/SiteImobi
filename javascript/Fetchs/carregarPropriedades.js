...
async function carregarPropriedades() {
    try {
        ...
        let propertiesUrl = 'http://localhost:5502/properties/';
        if (urlSearchParams.has('cityId')) {
            const cityId = urlSearchParams.get('cityId');
            propertiesUrl = `http://localhost:5502/properties/city/${cityId}`;
        }
        if (urlSearchParams.has('propertyTypeId')) {
            const propertyTypeId = urlSearchParams.get('propertyTypeId');
            propertiesUrl = `http://localhost:5502/properties/propertyType/${propertyTypeId}`
        }
